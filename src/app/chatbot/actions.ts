...
import type { Faq } from '@/backend/schemas/faq'; // Ensure Faq type is available
...
export async function generateChatResponse(
  ...
  try {
    ...
    // Fetch relevant FAQs (simplified: fetch all for now, in a real app, implement semantic search)
    ...
    // Fetch relevant document summaries (simplified: fetch some recent ones)
    ...
    const docSummaries: DocumentSummaryForPrompt[] = docSummariesCursor.map(doc => ({
        ...
        chunks: doc.chunks, // Assuming chunks have { text, summary }
